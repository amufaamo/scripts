...
add_seurat_oj_tcr <- function(seurat_object, tcr_csv_path) {
  ...
  PREFIX_TCR_ALPHA <- "TCR_TRA_"
  PREFIX_TCR_BETA  <- "TCR_TRB_"

  # --- ヘルパー関数 1: paste_existing_cols ---
  ...
  # --- ヘルパー関数 2: csv_to_tcr_pair_dataframe ---
  csv_to_tcr_pair_dataframe <- function(csv_path, sample_name = "temp_sample", id_name = "temp_id") {
    ...
    pair_list <- tryCatch({ scRepertoire::combineTCR(tcr_raw, samples = sample_name, ID = id_name, filterMulti = TRUE, removeNA = TRUE) }, error = function(e) { stop(e$message); NULL })
  ...
  # --- ヘルパー関数 3: csv_to_tcr_tra_dataframe ---
  csv_to_tcr_tra_dataframe <- function(csv_path){
    tcr_raw <- tryCatch({ readr::read_csv(csv_path, show_col_types = FALSE) }, error = function(e) { stop(e$message); NULL })
    if (is.null(tcr_raw) || !"chain" %in% names(tcr_raw)) { return(data.frame()) }
    TRA <- tcr_raw %>%
  ...
  # --- ヘルパー関数 4: csv_to_tcr_trb_dataframe ---
  ...
  merge_tcr <- function(pair, TRA, TRB){
    ...
    if (nrow(pair) == 0) { return(data.frame()) }
  ...
  # --- ヘルパー関数 6: tcr_csv_to_dataframe (メイン処理) ---
  tcr_csv_to_dataframe <- function(csv_path){
    ...
    if (nrow(pair_data) == 0) { return(data.frame()) }
    merged_df <- merge_tcr(pair_data, tra_data, trb_data)
    if (nrow(merged_df) == 0) { return(merged_df) }
    ...
    cols_to_remove <- c(paste0(PREFIX_TCR_ALPHA, c("raw_clonotype_id", "raw_consensus_id", "exact_subclonotype_id")), paste0(PREFIX_TCR_BETA, c("raw_clonotype_id", "raw_consensus_id", "exact_subclonotype_id")), paste0(PREFIX_TCR_PAIR, c("raw_clonotype_id", "raw_consensus_id", "exact_subclonotype_id")))
    ...
    processed_df <- merged_df %>%
      dplyr::mutate(
        ...
        exact_subclonotype_id = dplyr::case_when(
          trb_clonotype_col %in% names(.) & trb_subclonotype_col %in% names(.) & !is.na(.data[[trb_clonotype_col]]) & !is.na(.data[[trb_subclonotype_col]]) ~ stringr::str_c(.data[[trb_clonotype_col]], .data[[trb_subclonotype_col]], sep = "_"), TRUE ~ NA_character_
        )
      ) %>%
      dplyr::select(-dplyr::any_of(cols_to_remove_existing), -dplyr::any_of("raw_consensus_id")) %>%
      dplyr::relocate(dplyr::any_of(c("raw_clonotype_id", "exact_subclonotype_id")), dplyr::starts_with(PREFIX_TCR_PAIR), dplyr::starts_with(PREFIX_TCR_ALPHA), dplyr::starts_with(PREFIX_TCR_BETA), .before = tidyselect::everything())
    
    regions_nt <- c("fwr1_nt", "cdr1_nt", "fwr2_nt", "cdr2_nt", "fwr3_nt", "cdr3_nt", "fwr4_nt")
    regions_aa <- c("fwr1", "cdr1", "fwr2", "cdr2", "fwr3", "cdr3", "fwr4")
    processed_df <- processed_df %>%
      dplyr::mutate(
        !!paste0(PREFIX_TCR_ALPHA, "full_length_nt") := paste_existing_cols(., paste0(PREFIX_TCR_ALPHA, regions_nt)),
        !!paste0(PREFIX_TCR_BETA, "full_length_nt")  := paste_existing_cols(., paste0(PREFIX_TCR_BETA, regions_nt)),
        !!paste0(PREFIX_TCR_ALPHA, "full_length_aa") := paste_existing_cols(., paste0(PREFIX_TCR_ALPHA, regions_aa)),
        !!paste0(PREFIX_TCR_BETA, "full_length_aa")  := paste_existing_cols(., paste0(PREFIX_TCR_BETA, regions_aa))
      ) %>%
      dplyr::mutate(dplyr::across(dplyr::ends_with("full_length_nt") | dplyr::ends_with("full_length_aa"), ~ dplyr::na_if(., "")))
    ...
    if (!"barcode" %in% names(processed_df) && pair_barcode_col_rename %in% names(processed_df)) {
      processed_df <- processed_df %>% dplyr::rename(barcode = !!rlang::sym(pair_barcode_col_rename))
    }
    if ("barcode" %in% names(processed_df)) { processed_df <- processed_df %>% dplyr::relocate(barcode) }

    tra_key_col <- paste0(PREFIX_TCR_ALPHA, "chain"); trb_key_col <- paste0(PREFIX_TCR_BETA, "chain")
    if (tra_key_col %in% names(processed_df) && trb_key_col %in% names(processed_df)) {
      final_df <- processed_df %>% dplyr::filter(!is.na(.data[[tra_key_col]]) & !is.na(.data[[trb_key_col]]))
    } else { final_df <- processed_df }
    return(final_df)
  }
  ...
  if (nrow(tcr_df) == 0) {
    warning("Warning: TCR data frame is empty after processing. No TCR info will be added.")
  ...
  tcr_df_prepared <- tcr_df %>%
    mutate(barcode_key = str_replace(barcode, "-[0-9]+$", "")) %>%
    distinct(barcode_key, .keep_all = TRUE)
  ...
  combined_metadata <- combined_metadata %>% select(-seurat_barcode, -barcode_key)

  cat("--- Step 4: Seuratオブジェクトのメタデータを更新します ---\n")
  seurat_object@meta.data <- combined_metadata
  
  # CTgeneはscRepertoireのcombineTCRで作られる列名
  match_col <- paste0(PREFIX_TCR_PAIR, "CTgene")
  n_matched <- if (match_col %in% names(seurat_object@meta.data)) sum(!is.na(seurat_object@meta.data[[match_col]])) else 0
